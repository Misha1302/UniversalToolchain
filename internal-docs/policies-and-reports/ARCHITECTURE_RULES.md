...
`internal-docs/policies-and-reports/PROJECT_RULES.md` remains the coding standard. This file makes the non-negotiable architecture rules explicit so agents and contributors cannot accidentally narrow the framework into a Wist-specific or product-specific implementation.
...
See `internal-docs/policies-and-reports/SYNTAX_OWNERSHIP_RULES.md` for the full mandatory policy.
...
## 3.3 External contribution and artifact-route policy

External language composition must be contribution-planned rather than selected by concrete language or runtime-pack branches.

Mandatory rules:

- features describe user-visible capabilities; contributions own concrete pipeline participation;
- generic language SDK and runtime projects must not reference Wist assemblies, Wist feature IDs, Wist module aliases or another shipped language's names;
- extension packages are registered explicitly; generic authoring APIs must not scan the AppDomain, output directory or filesystem for plugins;
- single-owner slots fail on multiple owners unless the language definition declares an explicit replacement and may pin the expected previous owner;
- requirements should target capabilities when implementations are replaceable and concrete contribution IDs only when exact identity is semantically required;
- artifact transformations declare source kind, target kind and deterministic route cost; selected routes are included in the immutable plan hash and lock file;
- runtime providers must match the exact provider ID, version, Toolchain API, runtime contribution and backend routes selected by the plan;
- generic route execution must validate every transformer and final executor before running user source;
- legacy language adapters may translate a resolved contribution plan into an existing runtime format, but adapter metadata and translation logic must remain inside the owning language package.

Forbidden shortcuts:

- a central `feature id -> Wist module alias` or equivalent shipped-language map in generic code;
- silent first-provider-wins capability resolution;
- implicit replacement of a single-owner parser, type system, lowering, backend or runtime provider;
- executing a route different from the route stored in `LanguagePlan`;
- treating a descriptor-only route as executed evidence without a transformer/executor test.
...
The valid runtime activation chains are:
...
external language path:
LanguageDefinition
-> resolved contribution graph
...
Feature reports, capability reports, docs, CLI schema views, and catalogs may explain those chains, but they must not replace them.
