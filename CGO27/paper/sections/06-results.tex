...
\begin{table}[t]
\caption{Predeclared historical-corpus accounting. Included records are not added to policy-detection totals because the new exact-revision replay was resource-blocked before policy execution.}
...
\midrule
Included & 3 & provenance / feasibility only \\
...
This negative result changes the submission claim. The study can say that three author-selected historical defects map to the modeled fact vocabulary and have issue-defined pre-study evidence. It cannot report a historical P2 detection rate, call the set independent, or use the original confirmations as a new experiment. The exact-revision attempt is preserved as a resource-blocked receipt so a later machine can resume from the same source and dependency state without changing the frozen candidate table.
