...
| F-04 | major | Paper ablation tables were manually transcribed, allowing the article to diverge from raw evidence. | A deterministic renderer produces compact LaTeX from validated schema-v2 ablation JSON. The ablation runner byte-compares regenerated tables with committed anonymous paper sources. |
...
| F-06 | resolved | End-to-end case P07 exposed a mixed parameter/literal numeric-promotion defect under every policy. | Native arithmetic now applies deterministic binary numeric promotion before selecting the managed arithmetic method. P07 remains unchanged and returns to the valid-control set; the matrix requires 25/25 valid controls. |
...
| F-08 | blocker for performance | Shared CI timings and the observed 14.3% verifier-call reduction do not establish whole-compilation speedup. | The efficiency headline is forbidden. Decision-grade timing remains `BLOCKED_PINNED_MACHINE` until the frozen pinned-machine protocol is executed. |
...
The second strongest argument is performance relevance. The work currently supports correctness and fault-localization claims. It does not support a speedup claim, and the paper correctly treats the 14.3% call-count reduction as below threshold and not equivalent to whole-compilation time.
...
- Contract Experiment, Wist end-to-end, TensorRules, and schema-v2 ablations;
