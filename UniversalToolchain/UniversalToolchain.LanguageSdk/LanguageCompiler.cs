...
public sealed class LanguageCompiler
{
    ...
    public LanguageBuildResult Compile(LanguageDefinition definition)
    {
        ...
        (LanguagePackageDescriptor Package, LanguageContributionDescriptor Contribution)? runtimeProvider = null;
        ...
        var planHash = ComputePlanHash(definition, resolvedFeatures, effectiveContributions, effectiveRuntimeProvider, routes);
        ...
        IReadOnlyList<LanguageFeatureId> chain)
    {
        ...
        if (!_registry.TryGetFeature(id, out var package, out var feature))
        {
            diagnostics.Add(Error(
                "UTL1001",
                "planning",
        ...
        {
            output.Add(new ResolvedLanguageFeature(
                package.Id,
                package.Version,
                LanguageFeatureManifestSerializer.ComputeSha256(package),
                feature));
        }
    }
    ...
    private (LanguagePackageDescriptor Package, LanguageContributionDescriptor Contribution)? SelectRuntimeProvider(
        LanguageDefinition definition,
        IReadOnlySet<LanguageFeatureId> activeFeatureIds,
        ICollection<LanguageDiagnostic> diagnostics)
    {
        var candidates = _registry.GetRuntimeProviderContributions()
        ...
        IReadOnlyList<LanguageContributionId> chain)
    {
        ...
        if (!_registry.TryGetContribution(id, out var package, out var contribution))
        {
            diagnostics.Add(Error(
                "UTL2001",
                "planning",
        ...
        {
            output.Add(new ResolvedLanguageContribution(
                package.Id,
                package.Version,
                LanguageFeatureManifestSerializer.ComputeSha256(package),
                contribution));
    ...
    private static string ComputePlanHash(
        ...
        IReadOnlyList<ResolvedLanguageContribution> contributions,
        ResolvedLanguageContribution? runtimeProvider,
        IReadOnlyList<LanguageArtifactRoute> routes)
    {
