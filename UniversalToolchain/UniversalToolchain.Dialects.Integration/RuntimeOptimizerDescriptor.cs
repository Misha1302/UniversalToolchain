...
/// <summary>
///     Describes one explicitly registered optimizer entity resolvable by canonical identifier or alias.
/// </summary>
public sealed class RuntimeOptimizerDescriptor
{
    private readonly ReadOnlyCollection<string> _aliases;

    public RuntimeOptimizerDescriptor(string canonicalId, Type implementationType)
        : this(implementationType, null, canonicalId)
    {
    }

    public RuntimeOptimizerDescriptor(string canonicalId, Type implementationType, IEnumerable<string>? aliases)
    ...
    public RuntimeOptimizerDescriptor(Type implementationType, IEnumerable<string>? aliases = null, string? canonicalId = null)
    {
        ...
        if (!typeof(IAirOptimizer).IsAssignableFrom(implementationType))
            Thrower.Argument(nameof(implementationType), "Optimizer type must implement IAirOptimizer.");

        var resolvedCanonicalId = canonicalId ?? implementationType.FullName;
        if (string.IsNullOrWhiteSpace(resolvedCanonicalId))
            Thrower.Argument(nameof(canonicalId), "Optimizer canonical identifier must not be empty.");
