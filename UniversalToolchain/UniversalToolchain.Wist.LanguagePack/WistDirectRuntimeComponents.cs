...
{
    public static LanguageArtifactKind<WistInterpreterArtifact> Interpreter { get; } = new(WistArtifactKinds.InterpreterArtifact, WistArtifactKinds.InterpreterArtifactContract.ValueTypeIdentity!);
    public static LanguageArtifactKind<WistCilArtifact> Cil { get; } = new(WistArtifactKinds.CilArtifact, WistArtifactKinds.CilArtifactContract.ValueTypeIdentity!);
}

internal sealed class WistInterpreterArtifact(CompilationInput input, IAbstractIR air, SsaRouteReport? ssaReport = null)
{
...
internal sealed class WistCilArtifact(CompilationInput input, CilCompilationOutput compilation, SsaRouteReport? ssaReport = null)
{
    public CompilationInput Input { get; } = input ?? throw new ArgumentNullException(nameof(input));
...
internal static class WistDirectRuntimeComponents
{
    ...
    private static readonly LanguageRuntimeComponentTraits DirectTraits = new(LanguageComponentDeterminism.Unknown, LanguageComponentHostInterop.None);
        ...
        WistLanguageFeaturePackage package, LanguagePlan plan, LanguageRuntimeOptions options)
    {
        ...
        var capabilityErrors = capabilityCatalog.Diagnostics.Where(static diagnostic => diagnostic.Severity == ToolchainDiagnosticSeverity.Error).ToArray();
        if (capabilityErrors.Length != 0)
            throw new InvalidOperationException("Selected Wist capability catalog is invalid:" + Environment.NewLine + string.Join(Environment.NewLine, capabilityErrors.Select(static diagnostic => $"[{diagnostic.Code}] {diagnostic.Message}")));
    ...
    private static LanguageTransformerRegistration CreateOptimizerRegistration(WistRuntimeComponentDescriptor component) =>
        LanguageTransformerRegistration.Create<WistAirArtifact, WistAirArtifact>(component.ContributionId, WistDirectArtifactKinds.Air, WistDirectArtifactKinds.Air, DirectTraits,
            context => new WistDirectOptimizerTransformer(component.ContributionId, context.Plan, DirectTraits));

    private static LanguageTransformerRegistration CreateInterpreterBackendRegistration() =>
        LanguageTransformerRegistration.Create<WistAirArtifact, WistInterpreterArtifact>(WistContributionIds.InterpreterBackend, WistDirectArtifactKinds.Air, WistDirectBackendArtifactKinds.Interpreter, DirectTraits,
            context => new DelegateLanguageArtifactTransformer<WistAirArtifact, WistInterpreterArtifact>(WistContributionIds.InterpreterBackend, WistDirectArtifactKinds.Air, WistDirectBackendArtifactKinds.Interpreter,
                (source, _) => { WistIntrinsicPlanPolicy.Create(context.Plan, InterpreterBackend).Validate(source.Air); return new WistInterpreterArtifact(source.Input, source.Air, source.SsaReport); }, DirectTraits));

    private static LanguageExecutorRegistration CreateInterpreterExecutorRegistration() =>
        LanguageExecutorRegistration.Create<WistInterpreterArtifact, object?>(WistContributionIds.InterpreterBackend, InterpreterBackend, WistDirectBackendArtifactKinds.Interpreter, DirectTraits,
            _ => new DelegateLanguageArtifactExecutor<WistInterpreterArtifact, object?>(WistContributionIds.InterpreterBackend, InterpreterBackend, WistDirectBackendArtifactKinds.Interpreter,
                static (artifact, context) => { var environment = new ExecutionEnvironment(artifact.Input.ExternalBindings); var value = new InterpreterImpl().Execute(artifact.Air, environment); return WistRuntimeValueAdapterActivation.Normalize(context.Plan, value); }, DirectTraits));

    private static LanguageTransformerRegistration CreateCilBackendRegistration() =>
        LanguageTransformerRegistration.Create<WistAirArtifact, WistCilArtifact>(WistContributionIds.CilBackend, WistDirectArtifactKinds.Air, WistDirectBackendArtifactKinds.Cil, DirectTraits,
            context => new DelegateLanguageArtifactTransformer<WistAirArtifact, WistCilArtifact>(WistContributionIds.CilBackend, WistDirectArtifactKinds.Air, WistDirectBackendArtifactKinds.Cil,
                (source, _) => { WistIntrinsicPlanPolicy.Create(context.Plan, CilBackend).Validate(source.Air); return new WistCilArtifact(source.Input, new AbstractMethodsCompilerImpl().Compile(source.Air, source.Input), source.SsaReport); }, DirectTraits));

    private static LanguageExecutorRegistration CreateCilExecutorRegistration() =>
        LanguageExecutorRegistration.Create<WistCilArtifact, object?>(WistContributionIds.CilBackend, CilBackend, WistDirectBackendArtifactKinds.Cil, DirectTraits,
            _ => new DelegateLanguageArtifactExecutor<WistCilArtifact, object?>(WistContributionIds.CilBackend, CilBackend, WistDirectBackendArtifactKinds.Cil,
                static (artifact, context) => { var environment = new ExecutionEnvironment(artifact.Input.ExternalBindings); var value = new DynamicMethodExecutor().Execute(artifact.Compilation, environment); return WistRuntimeValueAdapterActivation.Normalize(context.Plan, value); }, DirectTraits));

    private static IAirOptimizer CreateOptimizer(LanguageContributionId contributionId, LanguagePlan plan, ISsaRouteReportSink? ssaReportSink = null)
    {
        if (contributionId == WistContributionIds.ArithmeticOptimizer) return new ArithmeticOptimizerModule();
        if (contributionId == WistContributionIds.BooleanOptimizer) return new BooleanOptimizerModule();
        if (contributionId == WistContributionIds.ComparisonIntrinsicOptimizer) return new ComparisonIntrinsicOptimizerModule();
        if (contributionId == WistContributionIds.EGraphOptimizer) return new EGraphOptimizerModule();
        if (contributionId == WistContributionIds.NativeCilOptimizer) return new NativeCilOptimizerModule();
        if (contributionId == WistContributionIds.NativeTypesOptimizer) return new NativeTypesOptimizerModule();
        if (contributionId == WistContributionIds.SsaOptimizer)
            return new SsaOptimizerModule(WistSsaPlanPolicy.CreateRuntimeOptions(plan, captureTrace: true), ssaReportSink ?? NullSsaRouteReportSink.Instance, []);
        throw new InvalidOperationException($"Wist optimizer contribution '{contributionId.Value}' has no direct runtime factory.");
    }

    private static IAbstractMethodsTranslator CreateMethodsTranslator()
    {
        using var services = new ServiceCollection().AddCoreIntrinsicServices().BuildServiceProvider();
        return new BytecodeToAbstractIrConverterImpl(services.GetRequiredService<IInstructionIntrinsicReader>(), services.GetRequiredService<IIntrinsicTypeStackProcessor>());
    }

    private static IOptimizerIntrinsicCapabilityContext CreateCapabilityContext(LanguagePlan plan, BackendId backend)
    {
        IOptimizerIntrinsicCapabilityContext inner = backend == CilBackend
            ? new OptimizerIntrinsicCapabilityContext(new CompilerIntrinsicCapabilitySetFactory().Create(new AbstractMethodsCompilerImpl()))
            : backend == InterpreterBackend
                ? new OptimizerIntrinsicCapabilityContext(new CompilerIntrinsicCapabilitySetFactory().Create(new AbstractIrToAbstractIrStub()))
    ...
    private sealed class WistDirectOptimizerTransformer(LanguageContributionId contributionId, LanguagePlan plan, LanguageRuntimeComponentTraits traits) : ILanguageArtifactTransformer<WistAirArtifact, WistAirArtifact>
    {
        ...
        {
            ArgumentNullException.ThrowIfNull(source); ArgumentNullException.ThrowIfNull(context);
            SsaRouteReport? report = source.SsaReport; IAirOptimizer optimizer; SsaRouteReportCollector.Capture? capture = null;
            if (ContributionId == WistContributionIds.SsaOptimizer) { var collector = new SsaRouteReportCollector(); capture = collector.BeginCapture(); optimizer = CreateOptimizer(ContributionId, plan, collector); }
            else optimizer = CreateOptimizer(ContributionId, plan);
            try
            {
    ...
    private sealed class TraitsOverrideTransformer<TSource, TTarget>(ILanguageArtifactTransformer<TSource, TTarget> inner, LanguageRuntimeComponentTraits traits) : ILanguageArtifactTransformer<TSource, TTarget>
    {
    ...
    private sealed class OwnedBytecodeTransformer(WistDirectBytecodeTransformer inner, ServiceProvider owner, LanguageRuntimeComponentTraits traits) : ILanguageArtifactTransformer<WistSemanticArtifact, WistBytecodeArtifact>, IDisposable
    {
        public LanguageContributionId ContributionId => inner.ContributionId;
        public LanguageArtifactKind<WistSemanticArtifact> TypedSourceKind => inner.TypedSourceKind;
        public LanguageArtifactKind<WistBytecodeArtifact> TypedTargetKind => inner.TypedTargetKind;
        public LanguageRuntimeComponentTraits TypedTraits { get; } = traits;
        public WistBytecodeArtifact Transform(WistSemanticArtifact source, LanguageArtifactTransformationContext context) => inner.Transform(source, context);
        public void Dispose() => owner.Dispose();
    }
}
