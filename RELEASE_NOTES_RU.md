...
Exact test manifest текущей ветки: **1,324 теста**: Core 500, Modules 292, Dialects 234, LanguageSdk.Generic 62, LanguageSdk 185, PlanFuzz 41 и 10 isolated integration cases. Перенос 58 generic implementation tests между owner-specific suites не уменьшает покрытие; 4 ранее существовавших Wist-free generic tests вместе с ними образуют 62-test canonical generic suite в Linux/Windows contract. Planner repair добавляет 18 focused LanguageSdk regressions и metamorphic controls для definition/executable ordering, route-wide pass feasibility, overflow-safe aggregate cost, semantic route/pass ambiguity, registration/input permutation, irrelevant alternatives и runtime-provider policy. Canonical build/test gate обязан подтвердить 0 failed и 0 skipped на exact candidate revision; более старые workflow receipts не подменяют exact-head verification.
